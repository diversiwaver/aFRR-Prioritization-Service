...
internal class TestPrioritizationModel
{
    ...
    [Test]
    public void PrioritizationModel_ShouldGetPrioritizedAssets_WhenGivenSignalAndThreshold(string modelVersion, decimal quantityThreshold)
    {
        // Assert
        ...
        // Act
        if (quantityThreshold > _assets.Sum(asset => asset.CapacityMw))
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => prioritizationModel.GetPrioritizedAssets(_assets, quantityThreshold), "Prioritization Model did not throw exception when given unreachable quantity threshold.");
        }
        else
        {
            retrievedAssets = prioritizationModel.GetPrioritizedAssets(_assets, quantityThreshold);

            totalRegulatedCapacity = retrievedAssets.Sum(asset => (asset.RegulationPercentage / 100) * asset.CapacityMw);

            // Arrange
            Assert.That(totalRegulatedCapacity, Is.EqualTo(quantityThreshold).Within(0.0001));
        }
    }
}
